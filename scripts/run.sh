...
# kind load docker-image --name=${current_context} cnpg-i-skip-initdb:${VERSION:-latest}
...
load_image_registry cnpg-i-skip-initdb:${VERSION:-latest}

# Patch the deployment to use the provided image
kubectl patch deployments.apps -n cnpg-system skip-initdb -p \
  "{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"cnpg-i-skip-initdb\",\"image\":\"${registry_name}:5000/cnpg-i-skip-initdb:${VERSION:-latest}\"}]}}}}"
